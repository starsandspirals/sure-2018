...
unsigned int h_Persons_default_variable_lastinfectedtime_data_iteration;
...
unsigned int h_Persons_s2_variable_lastinfectedtime_data_iteration;
...
xmachine_message_household_infection_list* h_household_infections;         /**< Pointer to message list on host*/
xmachine_message_household_infection_list* d_household_infections;         /**< Pointer to message list on device*/
xmachine_message_household_infection_list* d_household_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_household_infection_count;         /**< message list counter*/
int h_message_household_infection_output_type;   /**< message output type (single or optional)*/

/* church_infection Message variables */
xmachine_message_church_infection_list* h_church_infections;         /**< Pointer to message list on host*/
xmachine_message_church_infection_list* d_church_infections;         /**< Pointer to message list on device*/
xmachine_message_church_infection_list* d_church_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_church_infection_count;         /**< message list counter*/
int h_message_church_infection_output_type;   /**< message output type (single or optional)*/

/* transport_infection Message variables */
xmachine_message_transport_infection_list* h_transport_infections;         /**< Pointer to message list on host*/
xmachine_message_transport_infection_list* d_transport_infections;         /**< Pointer to message list on device*/
xmachine_message_transport_infection_list* d_transport_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_transport_infection_count;         /**< message list counter*/
int h_message_transport_infection_output_type;   /**< message output type (single or optional)*/

/* clinic_infection Message variables */
xmachine_message_clinic_infection_list* h_clinic_infections;         /**< Pointer to message list on host*/
xmachine_message_clinic_infection_list* d_clinic_infections;         /**< Pointer to message list on device*/
xmachine_message_clinic_infection_list* d_clinic_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_clinic_infection_count;         /**< message list counter*/
int h_message_clinic_infection_output_type;   /**< message output type (single or optional)*/

/* workplace_infection Message variables */
xmachine_message_workplace_infection_list* h_workplace_infections;         /**< Pointer to message list on host*/
xmachine_message_workplace_infection_list* d_workplace_infections;         /**< Pointer to message list on device*/
xmachine_message_workplace_infection_list* d_workplace_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_workplace_infection_count;         /**< message list counter*/
int h_message_workplace_infection_output_type;   /**< message output type (single or optional)*/

/* bar_infection Message variables */
xmachine_message_bar_infection_list* h_bar_infections;         /**< Pointer to message list on host*/
xmachine_message_bar_infection_list* d_bar_infections;         /**< Pointer to message list on device*/
xmachine_message_bar_infection_list* d_bar_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_bar_infection_count;         /**< message list counter*/
int h_message_bar_infection_output_type;   /**< message output type (single or optional)*/

/* school_infection Message variables */
xmachine_message_school_infection_list* h_school_infections;         /**< Pointer to message list on host*/
xmachine_message_school_infection_list* d_school_infections;         /**< Pointer to message list on device*/
xmachine_message_school_infection_list* d_school_infections_swap;    /**< Pointer to message swap list on device (used for holding optional messages)*/
/* Non partitioned and spatial partitioned message variables  */
int h_message_school_infection_count;         /**< message list counter*/
int h_message_school_infection_output_type;   /**< message output type (single or optional)*/
...
/** Person_updatelambdahh
 * Agent function prototype for updatelambdahh function of Person agent
 */
void Person_updatelambdahh(cudaStream_t &stream);

/** Person_updatelambdachu
 * Agent function prototype for updatelambdachu function of Person agent
 */
void Person_updatelambdachu(cudaStream_t &stream);

/** Person_updatelambdatr
 * Agent function prototype for updatelambdatr function of Person agent
 */
void Person_updatelambdatr(cudaStream_t &stream);

/** Person_updatelambdacl
 * Agent function prototype for updatelambdacl function of Person agent
 */
void Person_updatelambdacl(cudaStream_t &stream);

/** Person_updatelambdawp
 * Agent function prototype for updatelambdawp function of Person agent
 */
void Person_updatelambdawp(cudaStream_t &stream);

/** Person_updatelambdab
 * Agent function prototype for updatelambdab function of Person agent
 */
void Person_updatelambdab(cudaStream_t &stream);

/** Person_updatelambdasch
 * Agent function prototype for updatelambdasch function of Person agent
 */
void Person_updatelambdasch(cudaStream_t &stream);
...
void initialise(char * inputfile){
    ...
    h_Persons_default_variable_lastinfectedtime_data_iteration = 0;
    ...
    h_Persons_s2_variable_lastinfectedtime_data_iteration = 0;
	...
	int message_household_infection_SoA_size = sizeof(xmachine_message_household_infection_list);
	h_household_infections = (xmachine_message_household_infection_list*)malloc(message_household_infection_SoA_size);
	int message_church_infection_SoA_size = sizeof(xmachine_message_church_infection_list);
	h_church_infections = (xmachine_message_church_infection_list*)malloc(message_church_infection_SoA_size);
	int message_transport_infection_SoA_size = sizeof(xmachine_message_transport_infection_list);
	h_transport_infections = (xmachine_message_transport_infection_list*)malloc(message_transport_infection_SoA_size);
	int message_clinic_infection_SoA_size = sizeof(xmachine_message_clinic_infection_list);
	h_clinic_infections = (xmachine_message_clinic_infection_list*)malloc(message_clinic_infection_SoA_size);
	int message_workplace_infection_SoA_size = sizeof(xmachine_message_workplace_infection_list);
	h_workplace_infections = (xmachine_message_workplace_infection_list*)malloc(message_workplace_infection_SoA_size);
	int message_bar_infection_SoA_size = sizeof(xmachine_message_bar_infection_list);
	h_bar_infections = (xmachine_message_bar_infection_list*)malloc(message_bar_infection_SoA_size);
	int message_school_infection_SoA_size = sizeof(xmachine_message_school_infection_list);
	h_school_infections = (xmachine_message_school_infection_list*)malloc(message_school_infection_SoA_size);
  ...
  gpuErrchk( cudaMalloc( (void**) &d_xmachine_memory_SchoolMembership_keys, xmachine_memory_SchoolMembership_MAX* sizeof(uint)));
	...
	/* household_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_household_infections, message_household_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_household_infections_swap, message_household_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_household_infections, h_household_infections, message_household_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* church_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_church_infections, message_church_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_church_infections_swap, message_church_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_church_infections, h_church_infections, message_church_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* transport_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_transport_infections, message_transport_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_transport_infections_swap, message_transport_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_transport_infections, h_transport_infections, message_transport_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* clinic_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_clinic_infections, message_clinic_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_clinic_infections_swap, message_clinic_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_clinic_infections, h_clinic_infections, message_clinic_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* workplace_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_workplace_infections, message_workplace_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_workplace_infections_swap, message_workplace_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_workplace_infections, h_workplace_infections, message_workplace_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* bar_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_bar_infections, message_bar_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_bar_infections_swap, message_bar_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_bar_infections, h_bar_infections, message_bar_infection_SoA_size, cudaMemcpyHostToDevice));
	
	/* school_infection Message memory allocation (GPU) */
	gpuErrchk( cudaMalloc( (void**) &d_school_infections, message_school_infection_SoA_size));
	gpuErrchk( cudaMalloc( (void**) &d_school_infections_swap, message_school_infection_SoA_size));
	gpuErrchk( cudaMemcpy( d_school_infections, h_school_infections, message_school_infection_SoA_size, cudaMemcpyHostToDevice));
...
void cleanup(){
	...
	/* household_infection Message variables */
	free( h_household_infections);
	gpuErrchk(cudaFree(d_household_infections));
	gpuErrchk(cudaFree(d_household_infections_swap));
	
	/* church_infection Message variables */
	free( h_church_infections);
	gpuErrchk(cudaFree(d_church_infections));
	gpuErrchk(cudaFree(d_church_infections_swap));
	
	/* transport_infection Message variables */
	free( h_transport_infections);
	gpuErrchk(cudaFree(d_transport_infections));
	gpuErrchk(cudaFree(d_transport_infections_swap));
	
	/* clinic_infection Message variables */
	free( h_clinic_infections);
	gpuErrchk(cudaFree(d_clinic_infections));
	gpuErrchk(cudaFree(d_clinic_infections_swap));
	
	/* workplace_infection Message variables */
	free( h_workplace_infections);
	gpuErrchk(cudaFree(d_workplace_infections));
	gpuErrchk(cudaFree(d_workplace_infections_swap));
	
	/* bar_infection Message variables */
	free( h_bar_infections);
	gpuErrchk(cudaFree(d_bar_infections));
	gpuErrchk(cudaFree(d_bar_infections_swap));
	
	/* school_infection Message variables */
	free( h_school_infections);
	gpuErrchk(cudaFree(d_school_infections));
	gpuErrchk(cudaFree(d_school_infections_swap));
	

    /* Free temporary CUB memory if required. */
    
    if(d_temp_scan_storage_Person != nullptr){
      gpuErrchk(cudaFree(d_temp_scan_storage_Person));
      d_temp_scan_storage_Person = nullptr;
      temp_scan_storage_bytes_Person = 0;
    }
    
    if(d_temp_scan_storage_TBAssignment != nullptr){
...
PROFILE_SCOPED_RANGE("singleIteration");
  ...
  /* set all non partitioned, spatial partitioned and On-Graph Partitioned message counts to 0*/
	...
	h_message_household_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_household_infection_count, &h_message_household_infection_count, sizeof(int)));
	
	h_message_church_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_church_infection_count, &h_message_church_infection_count, sizeof(int)));
	
	h_message_transport_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_transport_infection_count, &h_message_transport_infection_count, sizeof(int)));
	
	h_message_clinic_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_clinic_infection_count, &h_message_clinic_infection_count, sizeof(int)));
	
	h_message_workplace_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_workplace_infection_count, &h_message_workplace_infection_count, sizeof(int)));
	
	h_message_bar_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_bar_infection_count, &h_message_bar_infection_count, sizeof(int)));
	
	h_message_school_infection_count = 0;
	//upload to device constant
	gpuErrchk(cudaMemcpyToSymbol( d_message_school_infection_count, &h_message_school_infection_count, sizeof(int)));
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdahh");
	Person_updatelambdahh(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdahh = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdachu");
	Person_updatelambdachu(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdachu = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdatr");
	Person_updatelambdatr(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdatr = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdacl");
	Person_updatelambdacl(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdacl = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdawp");
	Person_updatelambdawp(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdawp = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdab");
	Person_updatelambdab(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdab = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
    ...
    PROFILE_PUSH_RANGE("Person_updatelambdasch");
	Person_updatelambdasch(stream1);
    PROFILE_POP_RANGE();
#if defined(INSTRUMENT_AGENT_FUNCTIONS) && INSTRUMENT_AGENT_FUNCTIONS
	cudaEventRecord(instrument_stop);
	cudaEventSynchronize(instrument_stop);
	cudaEventElapsedTime(&instrument_milliseconds, instrument_start, instrument_stop);
	printf("Instrumentation: Person_updatelambdasch = %f (ms)\n", instrument_milliseconds);
#endif
	cudaDeviceSynchronize();
...
//constant setter
void set_DELTA(float* h_DELTA){
    gpuErrchk(cudaMemcpyToSymbol(DELTA, h_DELTA, sizeof(float)));
    memcpy(&h_env_DELTA, h_DELTA,sizeof(float));
}
...
//constant setter
void set_BAR_BETA0(float* h_BAR_BETA0){
    gpuErrchk(cudaMemcpyToSymbol(BAR_BETA0, h_BAR_BETA0, sizeof(float)));
    memcpy(&h_env_BAR_BETA0, h_BAR_BETA0,sizeof(float));
}

//constant getter
const float* get_BAR_BETA0(){
    return &h_env_BAR_BETA0;
}



//constant setter
void set_BAR_BETAA(float* h_BAR_BETAA){
    gpuErrchk(cudaMemcpyToSymbol(BAR_BETAA, h_BAR_BETAA, sizeof(float)));
    memcpy(&h_env_BAR_BETAA, h_BAR_BETAA,sizeof(float));
}

//constant getter
const float* get_BAR_BETAA(){
    return &h_env_BAR_BETAA;
}



//constant setter
void set_BAR_BETAS(float* h_BAR_BETAS){
    gpuErrchk(cudaMemcpyToSymbol(BAR_BETAS, h_BAR_BETAS, sizeof(float)));
    memcpy(&h_env_BAR_BETAS, h_BAR_BETAS,sizeof(float));
}

//constant getter
const float* get_BAR_BETAS(){
    return &h_env_BAR_BETAS;
}



//constant setter
void set_BAR_BETAAS(float* h_BAR_BETAAS){
    gpuErrchk(cudaMemcpyToSymbol(BAR_BETAAS, h_BAR_BETAAS, sizeof(float)));
    memcpy(&h_env_BAR_BETAAS, h_BAR_BETAAS,sizeof(float));
}
...
//constant setter
void set_E(float* h_E){
    gpuErrchk(cudaMemcpyToSymbol(E, h_E, sizeof(float)));
    memcpy(&h_env_E, h_E,sizeof(float));
}
...
__host__ unsigned int get_Person_default_variable_outsidetime(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_default_variable_outsidetime_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_default->outsidetime,
                    d_Persons_default->outsidetime,
                    count * sizeof(unsigned int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_default_variable_outsidetime_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_default->outsidetime[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access outsidetime for the %u th member of Person_default. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ int get_Person_default_variable_lastinfectedtime(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_default_variable_lastinfectedtime_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_default->lastinfectedtime,
                    d_Persons_default->lastinfectedtime,
                    count * sizeof(int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_default_variable_lastinfectedtime_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_default->lastinfectedtime[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access lastinfectedtime for the %u th member of Person_default. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ unsigned int get_Person_default_variable_bargoing(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_default_variable_bargoing_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_default->bargoing,
                    d_Persons_default->bargoing,
                    count * sizeof(unsigned int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_default_variable_bargoing_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_default->bargoing[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access bargoing for the %u th member of Person_default. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ unsigned int get_Person_default_variable_barday(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_default_variable_barday_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_default->barday,
                    d_Persons_default->barday,
                    count * sizeof(unsigned int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_default_variable_barday_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_default->barday[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access barday for the %u th member of Person_default. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ unsigned int get_Person_s2_variable_outsidetime(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_s2_variable_outsidetime_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_s2->outsidetime,
                    d_Persons_s2->outsidetime,
                    count * sizeof(unsigned int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_s2_variable_outsidetime_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_s2->outsidetime[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access outsidetime for the %u th member of Person_s2. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ int get_Person_s2_variable_lastinfectedtime(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_s2_variable_lastinfectedtime_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_s2->lastinfectedtime,
                    d_Persons_s2->lastinfectedtime,
                    count * sizeof(int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_s2_variable_lastinfectedtime_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_s2->lastinfectedtime[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access lastinfectedtime for the %u th member of Person_s2. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ unsigned int get_Person_s2_variable_bargoing(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_s2_variable_bargoing_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_s2->bargoing,
                    d_Persons_s2->bargoing,
                    count * sizeof(unsigned int),
                    cudaMemcpyDeviceToHost
                )
            );
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_s2_variable_bargoing_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_s2->bargoing[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access bargoing for the %u th member of Person_s2. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
__host__ unsigned int get_Person_s2_variable_barday(unsigned int index){
    ...
    if(count > 0 && index < count ){
        // If necessary, copy agent data from the device to the host in the default stream
        if(h_Persons_s2_variable_barday_data_iteration != currentIteration){
            gpuErrchk(
                cudaMemcpy(
                    h_Persons_s2->barday,
                    d_Persons_s2->barday,
            ...
            // Update some global value indicating what data is currently present in that host array.
            h_Persons_s2_variable_barday_data_iteration = currentIteration;
        }

        // Return the value of the index-th element of the relevant host array.
        return h_Persons_s2->barday[index];

    } else {
        fprintf(stderr, "Warning: Attempting to access barday for the %u th member of Person_s2. count is %u at iteration %u\n", index, count, currentIteration);
        // Otherwise we return a default value
        return 0;
...
void copy_single_xmachine_memory_Person_hostToDevice(xmachine_memory_Person_list * d_dst, xmachine_memory_Person * h_agent){
		...
		gpuErrchk(cudaMemcpy(d_dst->lastinfectedtime, &h_agent->lastinfectedtime, sizeof(int), cudaMemcpyHostToDevice));
...
void copy_partial_xmachine_memory_Person_hostToDevice(xmachine_memory_Person_list * d_dst, xmachine_memory_Person_list * h_src, unsigned int count){
    ...
    if (count > 0){
		...
		gpuErrchk(cudaMemcpy(d_dst->lastinfectedtime, h_src->lastinfectedtime, count * sizeof(int), cudaMemcpyHostToDevice));
...
void h_unpack_agents_Person_AoS_to_SoA(xmachine_memory_Person_list * dst, xmachine_memory_Person** src, unsigned int count){
	if(count > 0){
		for(unsigned int i = 0; i < count; i++){
			...
			dst->lastinfectedtime[i] = src[i]->lastinfectedtime;
...
unsigned int count_Person_default_id_variable(unsigned int count_value){
    ...
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->id),  thrust::device_pointer_cast(d_Persons_default->id) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_step_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->step),  thrust::device_pointer_cast(d_Persons_default->step) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_householdtime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->householdtime),  thrust::device_pointer_cast(d_Persons_default->householdtime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_churchtime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->churchtime),  thrust::device_pointer_cast(d_Persons_default->churchtime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_transporttime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->transporttime),  thrust::device_pointer_cast(d_Persons_default->transporttime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_clinictime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->clinictime),  thrust::device_pointer_cast(d_Persons_default->clinictime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_workplacetime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->workplacetime),  thrust::device_pointer_cast(d_Persons_default->workplacetime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_bartime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->bartime),  thrust::device_pointer_cast(d_Persons_default->bartime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_outsidetime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->outsidetime),  thrust::device_pointer_cast(d_Persons_default->outsidetime) + h_xmachine_memory_Person_default_count, count_value);
}
unsigned int min_Person_default_outsidetime_variable(){
    //min in default stream
    thrust::device_ptr<unsigned int> thrust_ptr = thrust::device_pointer_cast(d_Persons_default->outsidetime);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int count_Person_default_age_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->age),  thrust::device_pointer_cast(d_Persons_default->age) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_gender_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->gender),  thrust::device_pointer_cast(d_Persons_default->gender) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_householdsize_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->householdsize),  thrust::device_pointer_cast(d_Persons_default->householdsize) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_churchfreq_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->churchfreq),  thrust::device_pointer_cast(d_Persons_default->churchfreq) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_transportdur_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->transportdur),  thrust::device_pointer_cast(d_Persons_default->transportdur) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_household_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->household),  thrust::device_pointer_cast(d_Persons_default->household) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_busy_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->busy),  thrust::device_pointer_cast(d_Persons_default->busy) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_startstep_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->startstep),  thrust::device_pointer_cast(d_Persons_default->startstep) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_location_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->location),  thrust::device_pointer_cast(d_Persons_default->location) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_locationid_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->locationid),  thrust::device_pointer_cast(d_Persons_default->locationid) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_hiv_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->hiv),  thrust::device_pointer_cast(d_Persons_default->hiv) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_art_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->art),  thrust::device_pointer_cast(d_Persons_default->art) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_activetb_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->activetb),  thrust::device_pointer_cast(d_Persons_default->activetb) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_artday_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->artday),  thrust::device_pointer_cast(d_Persons_default->artday) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_default_infections_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->infections),  thrust::device_pointer_cast(d_Persons_default->infections) + h_xmachine_memory_Person_default_count, count_value);
}
...
int reduce_Person_default_lastinfectedtime_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_default->lastinfectedtime),  thrust::device_pointer_cast(d_Persons_default->lastinfectedtime) + h_xmachine_memory_Person_default_count);
}

int count_Person_default_lastinfectedtime_variable(int count_value){
    //count in default stream
    return (int)thrust::count(thrust::device_pointer_cast(d_Persons_default->lastinfectedtime),  thrust::device_pointer_cast(d_Persons_default->lastinfectedtime) + h_xmachine_memory_Person_default_count, count_value);
}
int min_Person_default_lastinfectedtime_variable(){
    //min in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_Persons_default->lastinfectedtime);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int count_Person_default_timevisiting_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->timevisiting),  thrust::device_pointer_cast(d_Persons_default->timevisiting) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int reduce_Person_default_bargoing_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_default->bargoing),  thrust::device_pointer_cast(d_Persons_default->bargoing) + h_xmachine_memory_Person_default_count);
}

unsigned int count_Person_default_bargoing_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->bargoing),  thrust::device_pointer_cast(d_Persons_default->bargoing) + h_xmachine_memory_Person_default_count, count_value);
}
unsigned int min_Person_default_bargoing_variable(){
    //min in default stream
    thrust::device_ptr<unsigned int> thrust_ptr = thrust::device_pointer_cast(d_Persons_default->bargoing);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int reduce_Person_default_barday_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_default->barday),  thrust::device_pointer_cast(d_Persons_default->barday) + h_xmachine_memory_Person_default_count);
}

unsigned int count_Person_default_barday_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->barday),  thrust::device_pointer_cast(d_Persons_default->barday) + h_xmachine_memory_Person_default_count, count_value);
}
unsigned int min_Person_default_barday_variable(){
    //min in default stream
    thrust::device_ptr<unsigned int> thrust_ptr = thrust::device_pointer_cast(d_Persons_default->barday);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_default_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int count_Person_default_schooltime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_default->schooltime),  thrust::device_pointer_cast(d_Persons_default->schooltime) + h_xmachine_memory_Person_default_count, count_value);
}
...
unsigned int count_Person_s2_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->id),  thrust::device_pointer_cast(d_Persons_s2->id) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_step_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->step),  thrust::device_pointer_cast(d_Persons_s2->step) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_householdtime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->householdtime),  thrust::device_pointer_cast(d_Persons_s2->householdtime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_churchtime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->churchtime),  thrust::device_pointer_cast(d_Persons_s2->churchtime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_transporttime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->transporttime),  thrust::device_pointer_cast(d_Persons_s2->transporttime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_clinictime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->clinictime),  thrust::device_pointer_cast(d_Persons_s2->clinictime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_workplacetime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->workplacetime),  thrust::device_pointer_cast(d_Persons_s2->workplacetime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_bartime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->bartime),  thrust::device_pointer_cast(d_Persons_s2->bartime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int reduce_Person_s2_outsidetime_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_s2->outsidetime),  thrust::device_pointer_cast(d_Persons_s2->outsidetime) + h_xmachine_memory_Person_s2_count);
}

unsigned int count_Person_s2_outsidetime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->outsidetime),  thrust::device_pointer_cast(d_Persons_s2->outsidetime) + h_xmachine_memory_Person_s2_count, count_value);
}
unsigned int min_Person_s2_outsidetime_variable(){
    //min in default stream
    thrust::device_ptr<unsigned int> thrust_ptr = thrust::device_pointer_cast(d_Persons_s2->outsidetime);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_s2_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int count_Person_s2_age_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->age),  thrust::device_pointer_cast(d_Persons_s2->age) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_gender_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->gender),  thrust::device_pointer_cast(d_Persons_s2->gender) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_householdsize_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->householdsize),  thrust::device_pointer_cast(d_Persons_s2->householdsize) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_churchfreq_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->churchfreq),  thrust::device_pointer_cast(d_Persons_s2->churchfreq) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_transportdur_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->transportdur),  thrust::device_pointer_cast(d_Persons_s2->transportdur) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_household_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->household),  thrust::device_pointer_cast(d_Persons_s2->household) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_busy_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->busy),  thrust::device_pointer_cast(d_Persons_s2->busy) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_startstep_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->startstep),  thrust::device_pointer_cast(d_Persons_s2->startstep) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_location_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->location),  thrust::device_pointer_cast(d_Persons_s2->location) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_locationid_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->locationid),  thrust::device_pointer_cast(d_Persons_s2->locationid) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_hiv_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->hiv),  thrust::device_pointer_cast(d_Persons_s2->hiv) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_art_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->art),  thrust::device_pointer_cast(d_Persons_s2->art) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_activetb_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->activetb),  thrust::device_pointer_cast(d_Persons_s2->activetb) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_artday_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->artday),  thrust::device_pointer_cast(d_Persons_s2->artday) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_infections_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->infections),  thrust::device_pointer_cast(d_Persons_s2->infections) + h_xmachine_memory_Person_s2_count, count_value);
}
...
int reduce_Person_s2_lastinfectedtime_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_s2->lastinfectedtime),  thrust::device_pointer_cast(d_Persons_s2->lastinfectedtime) + h_xmachine_memory_Person_s2_count);
}

int count_Person_s2_lastinfectedtime_variable(int count_value){
    //count in default stream
    return (int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->lastinfectedtime),  thrust::device_pointer_cast(d_Persons_s2->lastinfectedtime) + h_xmachine_memory_Person_s2_count, count_value);
}
int min_Person_s2_lastinfectedtime_variable(){
    //min in default stream
    thrust::device_ptr<int> thrust_ptr = thrust::device_pointer_cast(d_Persons_s2->lastinfectedtime);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_s2_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int count_Person_s2_timevisiting_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->timevisiting),  thrust::device_pointer_cast(d_Persons_s2->timevisiting) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int reduce_Person_s2_bargoing_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_s2->bargoing),  thrust::device_pointer_cast(d_Persons_s2->bargoing) + h_xmachine_memory_Person_s2_count);
}

unsigned int count_Person_s2_bargoing_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->bargoing),  thrust::device_pointer_cast(d_Persons_s2->bargoing) + h_xmachine_memory_Person_s2_count, count_value);
}
unsigned int min_Person_s2_bargoing_variable(){
    //min in default stream
    thrust::device_ptr<unsigned int> thrust_ptr = thrust::device_pointer_cast(d_Persons_s2->bargoing);
    size_t result_offset = thrust::min_element(thrust_ptr, thrust_ptr + h_xmachine_memory_Person_s2_count) - thrust_ptr;
    return *(thrust_ptr + result_offset);
}
...
unsigned int reduce_Person_s2_barday_variable(){
    //reduce in default stream
    return thrust::reduce(thrust::device_pointer_cast(d_Persons_s2->barday),  thrust::device_pointer_cast(d_Persons_s2->barday) + h_xmachine_memory_Person_s2_count);
}

unsigned int count_Person_s2_barday_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->barday),  thrust::device_pointer_cast(d_Persons_s2->barday) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_Person_s2_schooltime_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Persons_s2->schooltime),  thrust::device_pointer_cast(d_Persons_s2->schooltime) + h_xmachine_memory_Person_s2_count, count_value);
}
...
unsigned int count_TBAssignment_tbdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_TBAssignments_tbdefault->id),  thrust::device_pointer_cast(d_TBAssignments_tbdefault->id) + h_xmachine_memory_TBAssignment_tbdefault_count, count_value);
}
...
unsigned int count_Household_hhdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Households_hhdefault->id),  thrust::device_pointer_cast(d_Households_hhdefault->id) + h_xmachine_memory_Household_hhdefault_count, count_value);
}
...
unsigned int count_Household_hhdefault_active_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Households_hhdefault->active),  thrust::device_pointer_cast(d_Households_hhdefault->active) + h_xmachine_memory_Household_hhdefault_count, count_value);
}
...
unsigned int count_HouseholdMembership_hhmembershipdefault_household_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->household_id),  thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->household_id) + h_xmachine_memory_HouseholdMembership_hhmembershipdefault_count, count_value);
}
...
unsigned int count_HouseholdMembership_hhmembershipdefault_person_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->person_id),  thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->person_id) + h_xmachine_memory_HouseholdMembership_hhmembershipdefault_count, count_value);
}
...
unsigned int count_HouseholdMembership_hhmembershipdefault_household_size_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->household_size),  thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->household_size) + h_xmachine_memory_HouseholdMembership_hhmembershipdefault_count, count_value);
}
...
unsigned int count_HouseholdMembership_hhmembershipdefault_churchgoing_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->churchgoing),  thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->churchgoing) + h_xmachine_memory_HouseholdMembership_hhmembershipdefault_count, count_value);
}
...
unsigned int count_HouseholdMembership_hhmembershipdefault_churchfreq_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->churchfreq),  thrust::device_pointer_cast(d_HouseholdMemberships_hhmembershipdefault->churchfreq) + h_xmachine_memory_HouseholdMembership_hhmembershipdefault_count, count_value);
}
...
unsigned int count_Church_chudefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Churchs_chudefault->id),  thrust::device_pointer_cast(d_Churchs_chudefault->id) + h_xmachine_memory_Church_chudefault_count, count_value);
}
...
unsigned int count_Church_chudefault_size_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Churchs_chudefault->size),  thrust::device_pointer_cast(d_Churchs_chudefault->size) + h_xmachine_memory_Church_chudefault_count, count_value);
}
...
unsigned int count_Church_chudefault_active_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Churchs_chudefault->active),  thrust::device_pointer_cast(d_Churchs_chudefault->active) + h_xmachine_memory_Church_chudefault_count, count_value);
}
...
unsigned int count_ChurchMembership_chumembershipdefault_church_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_ChurchMemberships_chumembershipdefault->church_id),  thrust::device_pointer_cast(d_ChurchMemberships_chumembershipdefault->church_id) + h_xmachine_memory_ChurchMembership_chumembershipdefault_count, count_value);
}
...
unsigned int count_ChurchMembership_chumembershipdefault_household_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_ChurchMemberships_chumembershipdefault->household_id),  thrust::device_pointer_cast(d_ChurchMemberships_chumembershipdefault->household_id) + h_xmachine_memory_ChurchMembership_chumembershipdefault_count, count_value);
}
...
unsigned int count_Transport_trdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Transports_trdefault->id),  thrust::device_pointer_cast(d_Transports_trdefault->id) + h_xmachine_memory_Transport_trdefault_count, count_value);
}
...
unsigned int count_Transport_trdefault_active_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Transports_trdefault->active),  thrust::device_pointer_cast(d_Transports_trdefault->active) + h_xmachine_memory_Transport_trdefault_count, count_value);
}
...
unsigned int count_TransportMembership_trmembershipdefault_transport_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_TransportMemberships_trmembershipdefault->transport_id),  thrust::device_pointer_cast(d_TransportMemberships_trmembershipdefault->transport_id) + h_xmachine_memory_TransportMembership_trmembershipdefault_count, count_value);
}
...
unsigned int count_TransportMembership_trmembershipdefault_duration_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_TransportMemberships_trmembershipdefault->duration),  thrust::device_pointer_cast(d_TransportMemberships_trmembershipdefault->duration) + h_xmachine_memory_TransportMembership_trmembershipdefault_count, count_value);
}
...
unsigned int count_Clinic_cldefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Clinics_cldefault->id),  thrust::device_pointer_cast(d_Clinics_cldefault->id) + h_xmachine_memory_Clinic_cldefault_count, count_value);
}
...
unsigned int count_Workplace_wpdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Workplaces_wpdefault->id),  thrust::device_pointer_cast(d_Workplaces_wpdefault->id) + h_xmachine_memory_Workplace_wpdefault_count, count_value);
}
...
unsigned int count_WorkplaceMembership_wpmembershipdefault_person_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_WorkplaceMemberships_wpmembershipdefault->person_id),  thrust::device_pointer_cast(d_WorkplaceMemberships_wpmembershipdefault->person_id) + h_xmachine_memory_WorkplaceMembership_wpmembershipdefault_count, count_value);
}
...
unsigned int count_WorkplaceMembership_wpmembershipdefault_workplace_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_WorkplaceMemberships_wpmembershipdefault->workplace_id),  thrust::device_pointer_cast(d_WorkplaceMemberships_wpmembershipdefault->workplace_id) + h_xmachine_memory_WorkplaceMembership_wpmembershipdefault_count, count_value);
}
...
unsigned int count_Bar_bdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Bars_bdefault->id),  thrust::device_pointer_cast(d_Bars_bdefault->id) + h_xmachine_memory_Bar_bdefault_count, count_value);
}
...
unsigned int count_School_schdefault_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_Schools_schdefault->id),  thrust::device_pointer_cast(d_Schools_schdefault->id) + h_xmachine_memory_School_schdefault_count, count_value);
}
...
unsigned int count_SchoolMembership_schmembershipdefault_person_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_SchoolMemberships_schmembershipdefault->person_id),  thrust::device_pointer_cast(d_SchoolMemberships_schmembershipdefault->person_id) + h_xmachine_memory_SchoolMembership_schmembershipdefault_count, count_value);
}
...
unsigned int count_SchoolMembership_schmembershipdefault_school_id_variable(unsigned int count_value){
    //count in default stream
    return (unsigned int)thrust::count(thrust::device_pointer_cast(d_SchoolMemberships_schmembershipdefault->school_id),  thrust::device_pointer_cast(d_SchoolMemberships_schmembershipdefault->school_id) + h_xmachine_memory_SchoolMembership_schmembershipdefault_count, count_value);
}
...
/* Shared memory size calculator for agent function */
int Person_updatelambdahh_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_household_infection));
...
void Person_updatelambdahh(cudaStream_t &stream){
	...
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdahh_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdahh_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdahh, Person_updatelambdahh_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdahh_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdahh)
	//Reallocate   : false
	//Input        : household_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdahh<<<g, b, sm_size, stream>>>(d_Persons, d_household_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdahh agents in state s2 will be exceeded moving working agents to next state in function updatelambdahh\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdachu_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_church_infection));
...
void Person_updatelambdachu(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdachu_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdachu_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdachu, Person_updatelambdachu_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdachu_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdachu)
	//Reallocate   : false
	//Input        : church_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdachu<<<g, b, sm_size, stream>>>(d_Persons, d_church_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdachu agents in state s2 will be exceeded moving working agents to next state in function updatelambdachu\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdatr_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_transport_infection));
...
void Person_updatelambdatr(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdatr_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdatr_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdatr, Person_updatelambdatr_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdatr_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdatr)
	//Reallocate   : false
	//Input        : transport_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdatr<<<g, b, sm_size, stream>>>(d_Persons, d_transport_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdatr agents in state s2 will be exceeded moving working agents to next state in function updatelambdatr\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdacl_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_clinic_infection));
...
void Person_updatelambdacl(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdacl_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdacl_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdacl, Person_updatelambdacl_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdacl_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdacl)
	//Reallocate   : false
	//Input        : clinic_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdacl<<<g, b, sm_size, stream>>>(d_Persons, d_clinic_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdacl agents in state s2 will be exceeded moving working agents to next state in function updatelambdacl\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdawp_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_workplace_infection));
...
void Person_updatelambdawp(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdawp_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdawp_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdawp, Person_updatelambdawp_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdawp_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdawp)
	//Reallocate   : false
	//Input        : workplace_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdawp<<<g, b, sm_size, stream>>>(d_Persons, d_workplace_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdawp agents in state s2 will be exceeded moving working agents to next state in function updatelambdawp\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdab_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_bar_infection));
...
void Person_updatelambdab(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdab_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdab_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdab, Person_updatelambdab_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdab_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdab)
	//Reallocate   : false
	//Input        : bar_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdab<<<g, b, sm_size, stream>>>(d_Persons, d_bar_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdab agents in state s2 will be exceeded moving working agents to next state in function updatelambdab\n");
      exit(EXIT_FAILURE);
      }
...
/* Shared memory size calculator for agent function */
int Person_updatelambdasch_sm_size(int blockSize){
	int sm_size;
	sm_size = SM_START;
  //Continuous agent and message input has no partitioning
	sm_size += (blockSize * sizeof(xmachine_message_school_infection));
...
void Person_updatelambdasch(cudaStream_t &stream){
	...
	//APPLY FUNCTION FILTER
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, updatelambdasch_function_filter, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	updatelambdasch_function_filter<<<gridSize, blockSize, 0, stream>>>(d_Persons_s2, d_Persons);
	gpuErrchkLaunch();
	...
	//calculate the grid block size for main agent function
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, GPUFLAME_updatelambdasch, Person_updatelambdasch_sm_size, state_list_size);
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	b.x = blockSize;
	g.x = gridSize;
	
	sm_size = Person_updatelambdasch_sm_size(blockSize);
	...
	//MAIN XMACHINE FUNCTION CALL (updatelambdasch)
	//Reallocate   : false
	//Input        : school_infection
	//Output       : 
	//Agent Output : 
	GPUFLAME_updatelambdasch<<<g, b, sm_size, stream>>>(d_Persons, d_school_infections);
	gpuErrchkLaunch();
	...
	//check the working agents wont exceed the buffer size in the new state list
	if (h_xmachine_memory_Person_s2_count+h_xmachine_memory_Person_count > xmachine_memory_Person_MAX){
		printf("Error: Buffer size of updatelambdasch agents in state s2 will be exceeded moving working agents to next state in function updatelambdasch\n");
      exit(EXIT_FAILURE);
      }
...
void Household_hhupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_household_infection_count + h_xmachine_memory_Household_count > xmachine_message_household_infection_MAX){
		printf("Error: Buffer size of household_infection message will be exceeded in function hhupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_household_infection_output_type = single_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_household_infection_output_type, &h_message_household_infection_output_type, sizeof(int)));
	
	
	//MAIN XMACHINE FUNCTION CALL (hhupdate)
	//Reallocate   : false
	//Input        : location
	//Output       : household_infection
	//Agent Output : 
	GPUFLAME_hhupdate<<<g, b, sm_size, stream>>>(d_Households, d_locations, d_household_infections);
	gpuErrchkLaunch();
	...
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	h_message_household_infection_count += h_xmachine_memory_Household_count;
	//Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_household_infection_count, &h_message_household_infection_count, sizeof(int)));	
...
void Church_chuupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_church_infection_count + h_xmachine_memory_Church_count > xmachine_message_church_infection_MAX){
		printf("Error: Buffer size of church_infection message will be exceeded in function chuupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_church_infection_output_type = single_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_church_infection_output_type, &h_message_church_infection_output_type, sizeof(int)));
	
	
	//MAIN XMACHINE FUNCTION CALL (chuupdate)
	//Reallocate   : false
	//Input        : location
	//Output       : church_infection
	//Agent Output : 
	GPUFLAME_chuupdate<<<g, b, sm_size, stream>>>(d_Churchs, d_locations, d_church_infections);
	gpuErrchkLaunch();
	...
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	h_message_church_infection_count += h_xmachine_memory_Church_count;
	//Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_church_infection_count, &h_message_church_infection_count, sizeof(int)));	
...
void Transport_trupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_transport_infection_count + h_xmachine_memory_Transport_count > xmachine_message_transport_infection_MAX){
		printf("Error: Buffer size of transport_infection message will be exceeded in function trupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_transport_infection_output_type = single_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_transport_infection_output_type, &h_message_transport_infection_output_type, sizeof(int)));
	
	
	//MAIN XMACHINE FUNCTION CALL (trupdate)
	//Reallocate   : false
	//Input        : location
	//Output       : transport_infection
	//Agent Output : 
	GPUFLAME_trupdate<<<g, b, sm_size, stream>>>(d_Transports, d_locations, d_transport_infections);
	gpuErrchkLaunch();
	...
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	h_message_transport_infection_count += h_xmachine_memory_Transport_count;
	//Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_transport_infection_count, &h_message_transport_infection_count, sizeof(int)));	
...
void Clinic_clupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_clinic_infection_count + h_xmachine_memory_Clinic_count > xmachine_message_clinic_infection_MAX){
		printf("Error: Buffer size of clinic_infection message will be exceeded in function clupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_clinic_infection_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_clinic_infection_output_type, &h_message_clinic_infection_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_clinic_infection_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_clinic_infection_swaps<<<gridSize, blockSize, 0, stream>>>(d_clinic_infections); 
	gpuErrchkLaunch();
	...
	//Reallocate   : false
	//Input        : location
	//Output       : clinic_infection
	//Agent Output : 
	GPUFLAME_clupdate<<<g, b, sm_size, stream>>>(d_Clinics, d_locations, d_clinic_infections);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//clinic_infection Message Type Prefix Sum
	
	//swap output
	xmachine_message_clinic_infection_list* d_clinic_infections_scanswap_temp = d_clinic_infections;
	d_clinic_infections = d_clinic_infections_swap;
	d_clinic_infections_swap = d_clinic_infections_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_Clinic, 
        temp_scan_storage_bytes_Clinic, 
        d_clinic_infections_swap->_scan_input,
        d_clinic_infections_swap->_position,
        h_xmachine_memory_Clinic_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_clinic_infection_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_clinic_infection_messages<<<gridSize, blockSize, 0, stream>>>(d_clinic_infections, d_clinic_infections_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_clinic_infections_swap->_position[h_xmachine_memory_Clinic_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_clinic_infections_swap->_scan_input[h_xmachine_memory_Clinic_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_clinic_infection_count += scan_last_sum+1;
	}else{
		h_message_clinic_infection_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_clinic_infection_count, &h_message_clinic_infection_count, sizeof(int)));	
...
void Workplace_wpupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_workplace_infection_count + h_xmachine_memory_Workplace_count > xmachine_message_workplace_infection_MAX){
		printf("Error: Buffer size of workplace_infection message will be exceeded in function wpupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_workplace_infection_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_workplace_infection_output_type, &h_message_workplace_infection_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_workplace_infection_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_workplace_infection_swaps<<<gridSize, blockSize, 0, stream>>>(d_workplace_infections); 
	gpuErrchkLaunch();
	...
	//Reallocate   : false
	//Input        : location
	//Output       : workplace_infection
	//Agent Output : 
	GPUFLAME_wpupdate<<<g, b, sm_size, stream>>>(d_Workplaces, d_locations, d_workplace_infections);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//workplace_infection Message Type Prefix Sum
	
	//swap output
	xmachine_message_workplace_infection_list* d_workplace_infections_scanswap_temp = d_workplace_infections;
	d_workplace_infections = d_workplace_infections_swap;
	d_workplace_infections_swap = d_workplace_infections_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_Workplace, 
        temp_scan_storage_bytes_Workplace, 
        d_workplace_infections_swap->_scan_input,
        d_workplace_infections_swap->_position,
        h_xmachine_memory_Workplace_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_workplace_infection_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_workplace_infection_messages<<<gridSize, blockSize, 0, stream>>>(d_workplace_infections, d_workplace_infections_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_workplace_infections_swap->_position[h_xmachine_memory_Workplace_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_workplace_infections_swap->_scan_input[h_xmachine_memory_Workplace_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_workplace_infection_count += scan_last_sum+1;
	}else{
		h_message_workplace_infection_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_workplace_infection_count, &h_message_workplace_infection_count, sizeof(int)));	
...
void Bar_bupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_bar_infection_count + h_xmachine_memory_Bar_count > xmachine_message_bar_infection_MAX){
		printf("Error: Buffer size of bar_infection message will be exceeded in function bupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_bar_infection_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_bar_infection_output_type, &h_message_bar_infection_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_bar_infection_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_bar_infection_swaps<<<gridSize, blockSize, 0, stream>>>(d_bar_infections); 
	gpuErrchkLaunch();
	...
	//Reallocate   : false
	//Input        : location
	//Output       : bar_infection
	//Agent Output : 
	GPUFLAME_bupdate<<<g, b, sm_size, stream>>>(d_Bars, d_locations, d_bar_infections);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//bar_infection Message Type Prefix Sum
	
	//swap output
	xmachine_message_bar_infection_list* d_bar_infections_scanswap_temp = d_bar_infections;
	d_bar_infections = d_bar_infections_swap;
	d_bar_infections_swap = d_bar_infections_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_Bar, 
        temp_scan_storage_bytes_Bar, 
        d_bar_infections_swap->_scan_input,
        d_bar_infections_swap->_position,
        h_xmachine_memory_Bar_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_bar_infection_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_bar_infection_messages<<<gridSize, blockSize, 0, stream>>>(d_bar_infections, d_bar_infections_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_bar_infections_swap->_position[h_xmachine_memory_Bar_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_bar_infections_swap->_scan_input[h_xmachine_memory_Bar_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_bar_infection_count += scan_last_sum+1;
	}else{
		h_message_bar_infection_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_bar_infection_count, &h_message_bar_infection_count, sizeof(int)));	
...
void School_schupdate(cudaStream_t &stream){
	...
	//CONTINUOUS AGENT CHECK FUNCTION OUTPUT BUFFERS FOR OUT OF BOUNDS
	if (h_message_school_infection_count + h_xmachine_memory_School_count > xmachine_message_school_infection_MAX){
		printf("Error: Buffer size of school_infection message will be exceeded in function schupdate\n");
		exit(EXIT_FAILURE);
	}
	...
	//SET THE OUTPUT MESSAGE TYPE FOR CONTINUOUS AGENTS
	//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	h_message_school_infection_output_type = optional_message;
	gpuErrchk( cudaMemcpyToSymbol( d_message_school_infection_output_type, &h_message_school_infection_output_type, sizeof(int)));
	//message is optional so reset the swap
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, reset_school_infection_swaps, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	reset_school_infection_swaps<<<gridSize, blockSize, 0, stream>>>(d_school_infections); 
	gpuErrchkLaunch();
	...
	//Reallocate   : false
	//Input        : location
	//Output       : school_infection
	//Agent Output : 
	GPUFLAME_schupdate<<<g, b, sm_size, stream>>>(d_Schools, d_locations, d_school_infections);
	gpuErrchkLaunch();
	...
	//CONTINUOUS AGENTS SCATTER NON PARTITIONED OPTIONAL OUTPUT MESSAGES
	//school_infection Message Type Prefix Sum
	
	//swap output
	xmachine_message_school_infection_list* d_school_infections_scanswap_temp = d_school_infections;
	d_school_infections = d_school_infections_swap;
	d_school_infections_swap = d_school_infections_scanswap_temp;
	
    cub::DeviceScan::ExclusiveSum(
        d_temp_scan_storage_School, 
        temp_scan_storage_bytes_School, 
        d_school_infections_swap->_scan_input,
        d_school_infections_swap->_position,
        h_xmachine_memory_School_count, 
        stream
    );

	//Scatter
	cudaOccupancyMaxPotentialBlockSizeVariableSMem( &minGridSize, &blockSize, scatter_optional_school_infection_messages, no_sm, state_list_size); 
	gridSize = (state_list_size + blockSize - 1) / blockSize;
	scatter_optional_school_infection_messages<<<gridSize, blockSize, 0, stream>>>(d_school_infections, d_school_infections_swap);
	gpuErrchkLaunch();
	
	//UPDATE MESSAGE COUNTS FOR CONTINUOUS AGENTS WITH NON PARTITIONED MESSAGE OUTPUT 
	gpuErrchk( cudaMemcpy( &scan_last_sum, &d_school_infections_swap->_position[h_xmachine_memory_School_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk( cudaMemcpy( &scan_last_included, &d_school_infections_swap->_scan_input[h_xmachine_memory_School_count-1], sizeof(int), cudaMemcpyDeviceToHost));
	//If last item in prefix sum was 1 then increase its index to get the count
	if (scan_last_included == 1){
		h_message_school_infection_count += scan_last_sum+1;
	}else{
		h_message_school_infection_count += scan_last_sum;
	}
    //Copy count to device
	gpuErrchk( cudaMemcpyToSymbol( d_message_school_infection_count, &h_message_school_infection_count, sizeof(int)));	
