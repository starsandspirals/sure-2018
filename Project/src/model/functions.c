 ...
 * When the h_add_agent_Person_default() function is called, the data from the struct is immediately copied from the host to device.
...
__FLAME_GPU_INIT_FUNC__ void generatePersonInit()
{
	...
	for (int i = 0; i < h_agent_AoS_MAX; i++)
	{
		...
		xmachine_memory_Person *h_agent = h_allocate_agent_Person();
	...
	/*	for (unsigned int i = 0; i < xmachine_memory_Person_example_array_LENGTH; i++)
		...
		h_add_agent_Person_default(h_agent);
 ...
 * The AoS is converted to a SoA and copied to the device when h_add_agents_Person_default() is called.
 */
__FLAME_GPU_STEP_FUNC__ void generatePersonStep()
{
	...
	// unsigned int agent_remaining = xmachine_memory_Person_MAX - get_agent_Person_default_count();
	...
	// 		for (unsigned int j = 0; j < xmachine_memory_Person_example_array_LENGTH; j++) {
		...
		if (fp != nullptr)
		{
			...
			for (int index = 0; index < get_agent_Person_default_count(); index++)
			{
				...
				fprintf(
						...
						// 			get_Person_default_variable_example_vector(index).x,
						// 			get_Person_default_variable_example_vector(index).y,
						// 			get_Person_default_variable_example_array(index, 0),
						// 			get_Person_default_variable_example_array(index, 1)
				);
			}
...
__FLAME_GPU_FUNC__ int update(xmachine_memory_Person *agent, RNG_rand48 *rand48)
{
	...
	/*if(threadIdx.x + blockDim.x * blockIdx.x < 64){
		printf(
			...
			get_Person_agent_array_value<float>(agent->example_array, 0), 
			get_Person_agent_array_value<float>(agent->example_array, 1), 
			get_Person_agent_array_value<float>(agent->example_array, 2), 
			get_Person_agent_array_value<float>(agent->example_array, 3)
		);
	}*/
