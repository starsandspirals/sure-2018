...
// Allocating blocks of memory for each type of agent, and defining a constant
...
// Creating variables for the next unused ID for each agent type, so that they
...
__FLAME_GPU_INIT_FUNC__ void initialiseHost() {
  ...
  // Setting all of the variables that are input as parameters in the input.xml
  ...
  // Initialising all of the agent types with an id of 1 and allocating an array
  ...
  // Initialising the input file generated by Python preprocessing, from which
