...
GLuint prey_default1_tbo;
GLuint prey_default1_displacementTex;

GLuint predator_default2_tbo;
GLuint predator_default2_displacementTex;

GLuint grass_default3_tbo;
GLuint grass_default3_displacementTex;
...
__global__ void output_prey_agent_to_VBO(xmachine_memory_prey_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
__global__ void output_predator_agent_to_VBO(xmachine_memory_predator_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
__global__ void output_grass_agent_to_VBO(xmachine_memory_grass_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO( &prey_default1_tbo, &prey_default1_displacementTex, xmachine_memory_prey_MAX * sizeof( glm::vec4));
	
	createTBO( &predator_default2_tbo, &predator_default2_displacementTex, xmachine_memory_predator_MAX * sizeof( glm::vec4));
	
	createTBO( &grass_default3_tbo, &grass_default3_displacementTex, xmachine_memory_grass_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_prey_default1_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, prey_default1_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_prey_default1_count()/threads_per_tile);
		...
		output_prey_agent_to_VBO<<< grid, threads>>>(get_device_prey_default1_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(prey_default1_tbo));
	}
	
	if (get_agent_predator_default2_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, predator_default2_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_predator_default2_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		...
		output_predator_agent_to_VBO<<< grid, threads>>>(get_device_predator_default2_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(predator_default2_tbo));
	}
	
	if (get_agent_grass_default3_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, grass_default3_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_grass_default3_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		...
		output_grass_agent_to_VBO<<< grid, threads>>>(get_device_grass_default3_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(grass_default3_tbo));
	}
...
void display()
{
	...
	//Draw prey Agents in default1 state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, prey_default1_displacementTex);
	//loop
	for (int i=0; i< get_agent_prey_default1_count(); i++){
	...
	//Draw predator Agents in default2 state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, predator_default2_displacementTex);
	//loop
	for (int i=0; i< get_agent_predator_default2_count(); i++){
		glVertexAttrib1f(vs_mapIndex, (float)i);
	...
	//Draw grass Agents in default3 state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, grass_default3_displacementTex);
	//loop
	for (int i=0; i< get_agent_grass_default3_count(); i++){
		glVertexAttrib1f(vs_mapIndex, (float)i);
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case( 27) :
		...
		deleteTBO( &prey_default1_tbo);
		
		deleteTBO( &predator_default2_tbo);
		
		deleteTBO( &grass_default3_tbo);
